const {OPCUAClient, AttributeIds, StatusCode, check_flag,utils}=require('node-opcua')
                ...
                {nodeId:plcinfo.Tags[element],attributeId:AttributeIds.Value}
                    ...
                    for (var i = 0; i < nodeToRead.length; i++) {
                        ...
                        Object.keys(nodeidread[plcinfo.name]).forEach(key => {
                            ...
                            const value = nodeidread[plcinfo.name][key]
                            const keyParts = key.split('_').slice(1).join('_')
                            // console.log(key)
                            datafromplc[plcinfo.name][station][keyParts] = value.length === 2 ? value[1] : value
                          })
                    });
                     }
